...
- Exporter reads OpenClaw plain session transcripts (`~/.openclaw/agents/*/sessions/<uuid>.jsonl`) and Claude Code project transcripts (`~/.claude/projects/*/*.jsonl`). It does NOT read `*.trajectory.jsonl`: those exist for only a fraction of runs and undercount by an order of magnitude.
- Claude Code transcripts carry no cost; the exporter estimates API-equivalent cost from `ANTHROPIC_PRICING` (update that table when Anthropic pricing changes).
